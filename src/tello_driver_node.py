...
class TelloNode(tello.Tello):
    def __init__(self):
        ...
        self.pub_status = rospy.Publisher('/status', TelloStatus, queue_size=3)
        self.sub_status = rospy.Subscriber('/status',TelloStatus, self.cb_status_describe)
    ...
    def cb_status_describe(self,msg):
        heigh = msg.height_m
        #heigh = getattr( TelloStatus, 'height_m')
        #print(heigh)
